...
public class ProviderDaoImpl implements ProviderDao {
	...
	public String addPrescription(Prescription prescription) throws SQLException, ClassNotFoundException {
		...
		pst.setTimestamp(6,
				prescription.getWrittenOn() != null ? new java.sql.Timestamp(prescription.getWrittenOn().getTime())
						: new java.sql.Timestamp(System.currentTimeMillis()));

		pst.setTimestamp(7,
				prescription.getStartDate() != null ? new java.sql.Timestamp(prescription.getStartDate().getTime())
						: null);

		pst.setTimestamp(8,
				prescription.getEndDate() != null ? new java.sql.Timestamp(prescription.getEndDate().getTime()) : null);

		pst.setTimestamp(9,
				prescription.getCreatedAt() != null ? new java.sql.Timestamp(prescription.getCreatedAt().getTime())
						: new java.sql.Timestamp(System.currentTimeMillis()));
		if (prescription.getProcedure().getProcedureStatus() == ProcedureStatus.COMPLETED
				&& prescription.getProcedure().getType() == ProcedureType.SINGLE_DAY) {
	...
	@Override
	public String addPrescribedMedicines(PrescribedMedicines prescribedMedicines)
			throws ClassNotFoundException, SQLException {
		Connection con = ConnectionHelper.getConnection();

		String sql = "INSERT INTO prescribed_medicines ("
				+ "prescribed_id, prescription_id, medicine_name, dosage, duration, notes, "
				+ "start_date, end_date, created_at,medicine_type) " + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?,?)";
	...
	public String addProcedureDailyLog(ProcedureDailyLog log) throws ClassNotFoundException, SQLException {
		...
		pst.setDate(3, log.getLogDate() != null ? new java.sql.Date(log.getLogDate().getTime())
				: new java.sql.Date(System.currentTimeMillis()));

		pst.setString(4, log.getVitals());
		pst.setString(5, log.getNotes());

		pst.setTimestamp(6, log.getCreatedAt() != null ? new java.sql.Timestamp(log.getCreatedAt().getTime())
				: new java.sql.Timestamp(System.currentTimeMillis()));
	...
	public String generateNewProcedureId() throws SQLException, ClassNotFoundException {
		...
		if (rs.next()) {
			...
			if (latestId != null && latestId.startsWith("PROC")) {
				...
				currentNum++;
				newProcedureId = (currentNum <= 999) ? "PROC" + String.format("%03d", currentNum) : "PROC999";
			}
		}
	...
	public String generateNewPrescriptionId() throws SQLException, ClassNotFoundException {
		...
		if (rs.next()) {
			...
			if (latestId != null && latestId.startsWith("PRESC")) {
				...
				currentNum++;
				newPrescriptionId = (currentNum <= 999) ? "PRESC" + String.format("%03d", currentNum) : "PRESC999";
			}
		}
	...
	public String generateNewPrescribedMedicineId() throws SQLException, ClassNotFoundException {
		...
		if (rs.next()) {
			...
			if (latestId != null && latestId.startsWith("PMED")) {
				int currentNum = Integer.parseInt(latestId.substring(4));
				currentNum++;
				newPrescribedMedicineId = (currentNum <= 999) ? "PMED" + String.format("%03d", currentNum) : "PMED999";
			}
		}
	...
	public String generateNewProcedureTestId() throws SQLException, ClassNotFoundException {
		...
		if (rs.next()) {
			...
			if (latestId != null && latestId.startsWith("PTEST")) {
				int currentNum = Integer.parseInt(latestId.substring(5));
				currentNum++;
				newProcedureTestId = (currentNum <= 999) ? "PTEST" + String.format("%03d", currentNum) : "PTEST999";
			}
		}
	...
	public String generateNewProcedureLogId() throws ClassNotFoundException, SQLException {
		...
		if (rs.next()) {
			...
			if (latestId != null && latestId.startsWith("PLOG")) {
				int currentNum = Integer.parseInt(latestId.substring(4));
				currentNum++;
				newLogId = (currentNum <= 999) ? "PLOG" + String.format("%03d", currentNum) : "PLOG999";
			}
		}
	...
	public List<PrescribedMedicines> fetchMedicines(String prescriptionId) {
		...
		String sql = "SELECT prescribed_id, prescription_id, medicine_name, medicine_type, dosage, duration, "
				+ "       notes, start_date, end_date, created_at " + "FROM prescribed_medicines "
		...
		try (Connection conn = ConnectionHelper.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
			...
			try (ResultSet rs = ps.executeQuery()) {
				while (rs.next()) {
					PrescribedMedicines med = new PrescribedMedicines();

					med.setPrescribedId(rs.getString("prescribed_id"));
					...
					med.setStartDate(rs.getTimestamp("start_date"));
					med.setEndDate(rs.getTimestamp("end_date"));
					med.setCreatedAt(rs.getTimestamp("created_at"));

					// Set prescription reference by ID only
					Prescription prescription = new Prescription();
					prescription.setPrescriptionId(rs.getString("prescription_id"));
					med.setPrescription(prescription);

					// Handle ENUM for medicine type
					String type = rs.getString("medicine_type");
					if (type != null) {
						med.setType(MedicineType.valueOf(type.toUpperCase()));
					}
			...
			e.printStackTrace(); // Replace with proper logging strategy
		}
	...
	public List<ProcedureTest> fetchTests(String prescriptionId) {
		List<ProcedureTest> tests = new ArrayList<>();

		String sql = "SELECT test_id, prescription_id, test_name, test_date, result_summary, created_at "
				+ "FROM prescribed_tests " + "WHERE prescription_id = ?";
		...
		try (Connection conn = ConnectionHelper.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
			...
			try (ResultSet rs = ps.executeQuery()) {
				while (rs.next()) {
					...
					test.setResultSummary(rs.getString("result_summary"));
					test.setCreatedAt(rs.getTimestamp("created_at"));

					// Only setting prescriptionId within the Prescription object
					Prescription prescription = new Prescription();
					prescription.setPrescriptionId(rs.getString("prescription_id"));
	...
	@Override
	public String updatePrescription(Prescription prescription) throws SQLException, ClassNotFoundException {
		System.out.println("Remote prescription update called");
		...
		pst.setString(1, prescription.getProcedure().getProcedureId());
		pst.setString(2, prescription.getRecipient().gethId());
		pst.setString(3, prescription.getProvider().getProviderId());
		pst.setString(4, prescription.getDoctor().getDoctorId());
		...
		if (prescription.getProcedure().getProcedureStatus() == ProcedureStatus.COMPLETED
				&& prescription.getProcedure().getType() == ProcedureType.SINGLE_DAY) {
			pst.setString(9, prescription.getDoctor().getDoctorId());
		} else {
		...
		System.out.println("Prescription updated and returning from remote update");

		return rowsUpdated > 0 ? "updated" : "not found";
	}
	...
	public String updateMedicine(PrescribedMedicines m) throws ClassNotFoundException, SQLException {
		...
		String sql = "UPDATE prescribed_medicines SET "
				+ "prescription_id = ?, medicine_name = ?, dosage = ?, duration = ?, notes = ?, "
				+ "start_date = ?, end_date = ?, created_at = ?, medicine_type = ? " + "WHERE prescribed_id = ?";

		PreparedStatement pst = con.prepareStatement(sql);

		pst.setString(1, m.getPrescription().getPrescriptionId());
		pst.setString(2, m.getMedicineName());
		pst.setString(3, m.getDosage());
		pst.setString(4, m.getDuration());
		pst.setString(5, m.getNotes());

		pst.setTimestamp(6, m.getStartDate() != null ? new java.sql.Timestamp(m.getStartDate().getTime()) : null);

		pst.setTimestamp(7, m.getEndDate() != null ? new java.sql.Timestamp(m.getEndDate().getTime()) : null);

		pst.setTimestamp(8, m.getCreatedAt() != null ? new java.sql.Timestamp(m.getCreatedAt().getTime())
				: new java.sql.Timestamp(System.currentTimeMillis()));

		pst.setString(9, m.getType().name());
		pst.setString(10, m.getPrescribedId());
		...
		return rowsAffected > 0 ? "updated" : "not found";
	}
	...
	public String updateTest(ProcedureTest t) throws ClassNotFoundException, SQLException {
		...
		String sql = "UPDATE prescribed_tests SET "
				+ "prescription_id = ?, test_name = ?, test_date = ?, result_summary = ?, created_at = ? "
				+ "WHERE test_id = ?";

		PreparedStatement pst = con.prepareStatement(sql);

		pst.setString(1, t.getPrescription().getPrescriptionId());
		pst.setString(2, t.getTestName());

		if (t.getTestDate() != null) {
			pst.setDate(3, new java.sql.Date(t.getTestDate().getTime()));
		} else {
			pst.setDate(3, new java.sql.Date(System.currentTimeMillis())); // fallback
		}

		pst.setString(4, t.getResultSummary());

		if (t.getCreatedAt() != null) {
			pst.setTimestamp(5, new java.sql.Timestamp(t.getCreatedAt().getTime()));
		} else {
			pst.setTimestamp(5, new java.sql.Timestamp(System.currentTimeMillis()));
		}

		pst.setString(6, t.getTestId());
	...
	public String updateLog(ProcedureDailyLog log) throws ClassNotFoundException, SQLException {
	    Connection con = ConnectionHelper.getConnection();
	    String sql = "UPDATE procedure_daily_log SET "
	               + "procedure_id = ?, "
	               + "log_date = ?, "
	               + "vitals = ?, "
	               + "notes = ?, "
	               + "created_at = ?, "
	               + "logged_by = ? "
	               + "WHERE log_id = ?";  // Primary key used to identify which row to update

	    PreparedStatement pst = con.prepareStatement(sql);

	    pst.setString(1, log.getMedicalProcedure().getProcedureId());

	    pst.setDate(2, log.getLogDate() != null 
	        ? new java.sql.Date(log.getLogDate().getTime()) 
	        : new java.sql.Date(System.currentTimeMillis()));

	    pst.setString(3, log.getVitals());
	    pst.setString(4, log.getNotes());

	    pst.setTimestamp(5, log.getCreatedAt() != null 
	        ? new java.sql.Timestamp(log.getCreatedAt().getTime()) 
	        : new java.sql.Timestamp(System.currentTimeMillis()));

	    pst.setString(6, log.getloggedDoctor().getDoctorId());

	    pst.setString(7, log.getLogId()); // log_id as the WHERE condition
